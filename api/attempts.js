...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const { course, total, correct, pct, bySubject, answers, elapsedSeconds, date } = req.body || {};
    ...
    const [attempt] = await sql`
      INSERT INTO attempts (user_id, course, total, correct, pct, by_subject, answers, elapsed_seconds, attempt_date)
      VALUES (
        ${user.id}, ${course}, ${total}, ${correct}, ${pct},
        ${JSON.stringify(bySubject)}, ${answers ? JSON.stringify(answers) : null},
        ${elapsedSeconds || null}, ${attemptDate.toISOString()}
      )
      RETURNING
